...
import { Modal as AccessibleModal } from './ui';
      ...
      <div className="panel mb-4">
        <div className="panel-header">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setRootTab('board')} className={`btn-ghost h-10 px-3 ${rootTab === 'board' ? 'is-active' : ''}`}><BriefcaseBusiness size={15} />{t.board}</button>
            <button type="button" onClick={() => setRootTab('escrows')} className={`btn-ghost h-10 px-3 ${rootTab === 'escrows' ? 'is-active' : ''}`}><WalletCards size={15} />{t.mine}</button>
      ...
      {rootTab === 'board' ? (
        <>
          <div className="mb-3 grid grid-cols-3 gap-2 lg:hidden">
            {(['gigs', 'chat', 'escrow'] as MobilePane[]).map((pane) => <button key={pane} type="button" onClick={() => setMobilePane(pane)} className={`btn-ghost h-9 px-2 text-xs ${mobilePane === pane ? 'is-active' : ''}`}>{pane === 'gigs' ? t.board : pane === 'chat' ? t.chat : t.escrow}</button>)}
          </div>
...
function GigBoardPanel({ t, gigs, selectedGigId, search, typeFilter, categoryFilter, canCreate, onSearch, onType, onCategory, onSelect, onCreate }: { t: typeof text.en | typeof text.tr; gigs: GigRecord[]; selectedGigId: bigint | null; search: string; typeFilter: 'all' | ListingType; categoryFilter: 'all' | GigCategory; canCreate: boolean; onSearch: (value: string) => void; onType: (value: 'all' | ListingType) => void; onCategory: (value: 'all' | GigCategory) => void; onSelect: (gig: GigRecord) => void; onCreate: () => void }) {
  return <div className="panel min-h-[680px] p-3">
    ...
    <div className="mt-3 grid max-h-[500px] gap-2 overflow-y-auto pr-1">{gigs.length === 0 ? <Empty text={t.noGigs} /> : gigs.map((gig) => <button type="button" key={gig.id.toString()} onClick={() => onSelect(gig)} className={`chat-card p-3 text-left ${gig.id === selectedGigId ? 'is-active' : ''}`}><div className="flex items-center justify-between gap-2"><Badge>{gig.listingType === 0 ? t.workRequest : t.serviceOffer}</Badge><span className="text-[11px] text-zinc-500">#{gig.id.toString()}</span></div><h3 className="mt-2 text-sm font-semibold text-zinc-100">{gig.title}</h3><p className="mt-1 line-clamp-2 text-xs leading-5 text-zinc-500">{gig.description}</p><div className="mt-3 flex items-center justify-between text-xs"><span className="text-zinc-500">{categoryLabels[gig.category]}</span><span className="font-mono text-emerald-200">{formatEther(gig.suggestedBudget)} USDC</span></div></button>)}</div>
  </div>;
}
...
function MyEscrowsPanel({ t, language, items, filter, selected, onFilter, onSelect }: { t: typeof text.en | typeof text.tr; language: Language; items: EscrowRecord[]; filter: EscrowFilter; selected: bigint | null; onFilter: (filter: EscrowFilter) => void; onSelect: (id: bigint) => void }) {
  return <div className="panel min-h-[620px]"><div className="panel-header"><div><p className="eyebrow">Portfolio</p><h2 className="panel-title">{t.mine}</h2></div><div className="segmented-control grid-cols-2"><button type="button" onClick={() => onFilter('active')} className={`segment-option ${filter === 'active' ? 'segment-option-active' : ''}`}>{t.active}</button><button type="button" onClick={() => onFilter('history')} className={`segment-option ${filter === 'history' ? 'segment-option-active' : ''}`}>{t.history}</button></div></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.length === 0 ? <Empty text={t.noEscrows} /> : items.map((item) => <button type="button" key={item.id.toString()} onClick={() => onSelect(item.id)} className={`chat-card p-4 text-left ${item.id === selected ? 'is-active' : ''}`}><div className="flex items-center justify-between"><span className="font-mono text-xs text-zinc-500">#{item.id.toString()}</span><StatusBadge status={item.status} /></div><p className="mt-3 text-lg font-semibold text-white">{formatEther(item.amount)} USDC</p><div className="mt-3 grid gap-1 text-xs text-zinc-500"><span>{t.payer}: {short(item.payer)}</span><span>{t.provider}: {short(item.provider)}</span><span>{t.deadline}: {date(item.deadline, language)}</span></div></button>)}</div></div>;
}
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) { return <AccessibleModal label={title} onClose={onClose}><div className="modal-panel max-h-[90vh] overflow-y-auto"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><ShieldCheck size={18} className="text-emerald-300" /><h2 className="text-lg font-semibold text-white">{title}</h2></div><button type="button" onClick={onClose} className="btn-ghost h-9 w-9" aria-label="Close"><X size={16} /></button></div>{children}</div></AccessibleModal>; }
