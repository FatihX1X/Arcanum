    ...
    <section className="min-w-0">
      ...
      {rootTab === 'board' ? (
        <>
          ...
          {selectedEscrow && mobilePane !== 'escrow' ? <button type="button" onClick={() => setMobilePane('escrow')} className="fixed bottom-4 left-4 right-4 z-30 flex items-center justify-between rounded-lg border border-emerald-400/30 bg-zinc-950 px-4 py-3 shadow-2xl lg:hidden"><span className="text-sm font-semibold text-white">Escrow #{selectedEscrow.id.toString()}</span><StatusBadge status={selectedEscrow.status} /></button> : null}
        </>
      ) : (
...
function MessageBubble({ t, language, message, conversation, gig, viewer, proposal, onProposalAction }: { t: typeof text.en | typeof text.tr; language: Language; message: EscrowChatMessage; conversation: EscrowConversation; gig: GigRecord; viewer?: `0x${string}`; proposal?: EscrowProposal; onProposalAction: (proposal: EscrowProposal, action: 'accept' | 'withdraw' | 'fund') => void }) {
  ...
  return <article className={`max-w-[92%] rounded-lg border p-3 ${outgoing ? 'ml-auto border-emerald-400/25 bg-emerald-400/10' : 'border-zinc-800 bg-zinc-950'}`}><EncryptedText t={t} message={message} conversation={conversation} gig={gig} viewer={viewer} />{proposal ? <ProposalCard t={t} address={viewer} proposal={proposal} onAction={onProposalAction} /> : null}<div className="mt-2 text-right text-[11px] text-zinc-500">{date(message.timestamp, language)}</div></article>;
}
